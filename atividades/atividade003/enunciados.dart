...
// terminam com a letra 'R' (maiúscula ou minúscula). As palavras devem ser separadas por um espaço na frase final.
...
// Dada uma lista de strings que supostamente são endereços de email, filtre e colete
// em uma nova lista apenas os emails que contêm exatamente um caractere '@' e terminam com ".com".

// 10. Reversão Manual da Ordem dos Elementos
// Dada uma lista de strings, crie uma nova lista que contenha os mesmos elementos,
// mas na ordem inversa da lista original. O uso de List.reversed é permitido para fins de comparação,
...
// Dada uma lista de Maps, onde cada Map representa um produto com as chaves "nome" (String), "estoque" (int) e "ativo" (bool),
// calcule a soma total do estoque de todos os produtos que estão com o status "ativo" igual a true.

// 13. Mapeamento de Mapas para Strings Formatadas
// Dada a lista de Maps do exercício anterior (Produtos), crie uma nova lista de strings onde cada string
// é um resumo formatado do produto, no formato: "Produto [Nome] - Estoque: [Estoque]".

// 14. Encontrando o Item Mais Caro
// Dada uma lista de Maps, onde cada Map representa um item com as chaves "nome" (String) e "preco" (double),
// encontre e imprima o nome do item que possui o maior preço.

// 15. Contagem de Ocorrências de Status
// Dada uma lista de Maps, onde cada Map representa uma tarefa com a chave "status" (String)
//, conte e imprima o número total de tarefas para cada status distinto.

// 16. Interseção de Duas Listas
// Dadas duas listas de inteiros, crie uma nova lista contendo apenas
// os elementos que estão presentes em ambas as listas (a interseção).

// 17. Normalização e Classificação de Dados
// Dada uma lista de strings com valores duplicados e em ordem aleatória,
// o objetivo é primeiro remover todas as duplicatas e, em seguida, ordenar a lista resultante em ordem alfabética.
...
// O processo envolve: 1) calcular a média; 2) calcular a diferença entre cada número e a média, elevando ao quadrado;
// 3) somar os resultados; 4) dividir pela contagem de elementos menos 1; 5) tirar a raiz quadrada do resultado.

// 20. Particionamento de Lista em Positivos e Não-Positivos
// Dada uma lista de números inteiros (positivos, negativos e zero),
// crie uma nova lista onde todos os números positivos aparecem primeiro, seguidos pelos números não-positivos (zero e negativos).
